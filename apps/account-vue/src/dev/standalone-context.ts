...
/**
 * A minimal shell replacement for standalone development.
 *
 * It implements the routing half of the contract against the real URL, so
 * `localhost:3002/banking/accounts/acc_123` behaves in standalone exactly as it
 * does inside the shell — otherwise standalone mode would quietly diverge from
...
export async function createStandaloneContext(): Promise<MfeMountContext> {
  ...
  return {
    ...
    events: createEventBus({ source: 'account', onEmit: (event) => console.info('[event]', event) }),
    navigate: (path) => {
...
async function login(): Promise<LoginResponse> {
  const response = await fetch(new URL('/auth/login', API_BASE_URL), {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-application-id': 'account' },
    credentials: 'include',
